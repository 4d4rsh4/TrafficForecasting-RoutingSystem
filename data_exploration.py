...
# Let's look at Sensor 0, for the next 288 steps (roughly 24 hours)
sensor_idx = 0
time_steps = 288 
...
def adaptive_signal_control(predicted_flow, hour, day):
    # hour is 0-23, day is 0-6
    
    # Logic for Weekend (Sat=5, Sun=6)
    if day >= 5:
        if predicted_flow > 300: return 50
        else: return 30
        
    # Logic for Weekday Rush Hour (7-9 AM or 4-7 PM)
    if (7 <= hour <= 9) or (16 <= hour <= 19):
        if predicted_flow > 400: return 80 # Longer lights for peak rush
        else: return 60
        
    # Default Night/Off-peak
    return 30

# 1. Pick a sample index from the TEST set
sample_idx = 0 

# 2. Get the prediction
prediction_15m = y_pred[sample_idx, 0, 0] 

# 3. Find the actual time for this sample
# We add 'split' because X_test starts after the training data
# We add '12' because we are predicting 1 hour into the future
actual_time = time_index[split + sample_idx + 12]
h = actual_time.hour
d = actual_time.dayofweek

# 4. Call the function with all 3 parameters
green_time = adaptive_signal_control(prediction_15m, h, d)

print(f"Time: {actual_time} (Hour: {h}, Day: {d})")
print(f"Next 15m Predicted Flow: {prediction_15m:.0f} cars")
print(f"Setting Green Light to: {green_time} seconds")
...
def get_prediction_by_user_input(input_day_name, input_hour):
    ...
    input_hour: 0-23
    """
    # 1. Map day names to numbers (0=Monday, 6=Sunday)
    day_map = {
    ...
    # 2. Find the first index in the data that matches this day and hour
    ...
    # 3. Get the "Window" (the past 12 steps leading up to this hour)
    ...
    # 5. Adaptive Control
    green_time = adaptive_signal_control(flow_prediction, input_hour, target_day)

    print(f"\n--- MANUAL OVERRIDE SYSTEM ---")
    print(f"Day: {input_day_name} | Hour: {input_hour}:00")
    print(f"Retrieved historical sensor context from: {time_index[found_idx]}")
    print(f"Predicted Flow for next 15 mins: {flow_prediction:.1f} vehicles")
    print(f"RESULT: Setting Green Light to {green_time} seconds.")

# --- INTERACTIVE SECTION ---
# You can change these values manually to test different scenarios
my_day = "Monday" 
my_hour = 17 # 5 PM (Rush Hour)

get_prediction_by_user_input(my_day, my_hour)

my_day_2 = "Sunday"
my_hour_2 = 3 # 3 AM (Night)

get_prediction_by_user_input(my_day_2, my_hour_2)
